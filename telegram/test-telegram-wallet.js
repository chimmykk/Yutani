...
// Test wallet creation and encryption functionality for Telegram bot
async function testTelegramWalletCreation() {
  console.log('🧪 Testing Telegram Wallet Creation and Encryption...\n');
  ...
  try {
    ...
    const testPassphrase = 'my_secure_telegram_passphrase_123';
    ...
    const filename = `telegram_wallet_${walletData.address.slice(0, 8)}_${Date.now()}`;
    ...
    // Test 7: Test passphrase length validation
    console.log('\n7️⃣ Testing passphrase length validation...');
    const shortPassphrase = '123';
    if (shortPassphrase.length < config.MIN_PASSPHRASE_LENGTH) {
      console.log(`✅ Correctly identified short passphrase (${shortPassphrase.length} < ${config.MIN_PASSPHRASE_LENGTH})`);
    } else {
      console.log('❌ Passphrase length validation failed!');
    }

    console.log('\n🎉 All Telegram bot tests passed! Wallet creation and encryption is working correctly.');
    console.log('\n📋 Summary:');
